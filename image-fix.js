(() => {
  'use strict';
  const STYLE_ID='rock-product-image-surface-v8';
  const wired=new WeakSet();
  const IMAGE_VERSION='20260916-original-canvas-2';
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .product-card .product-image.rock-wide-catalog{width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:cover!important;object-position:left center!important;}
  ...
  function classifyCanvas(img){
    try{
      if(img.naturalWidth && img.naturalHeight){
        const ratio=img.naturalWidth/img.naturalHeight;
        img.classList.toggle('rock-wide-catalog',ratio>1.18);
      }
    }catch(_){ }
  }
  function ready(img){const v=visual(img);classifyCanvas(img);if(v){v.classList.remove('image-missing');v.classList.add('image-ready')}}
  function missing(img){img.classList.remove('rock-wide-catalog');const v=visual(img);if(v){v.classList.remove('image-ready');v.classList.add('image-missing')}}
